...
\chapter{Introduction: What Is \latex?}%
...
\subsection{The \env{document} Environment and the Preamble}%
...
\subsubsection{The \env{equation} Environment}%
...
\section{Specifying Equation Numbers or Names}%
...
\subsection{A \env{Theo1} Environment}
...
\subsection{A \env{Theo2} Environment}
...
\subsection{A \env{Lemma} Environment}
...
\chapter{Figures and Tables---\protect\LaTeX's Float Environments}%
...
\subsection{Fragile Commands and Moving Arguments}%
...
Notice that we didn't specify the \texttt{.eps} extension in the
filename argument to the \com{includegraphics} command.  By dropping
the extension, we can typeset this document with PDF\LaTeX\ by simply
changing the driver argument in the line that loads the
\package{graphics} package (provided that we have a PDF version of the
graphic, of course).  The \com{includegraphics} package searches for
different graphic formats depending on the driver it's using.


\subsection{\LaTeX\ Diagrams}
...
\cmidrule(r){2-3}\cmidrule(c){4-5}\cmidrule(l){6-7}
...
%%% Local Variables: 
%%% mode: latex
%%% TeX-master: "sample-clinic-report"
%%% End: 
